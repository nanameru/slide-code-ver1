...
/// Check if a bash command string is safe to execute
pub fn is_safe_bash_command(command: &str) -> bool {
    if let Some(tree) = try_parse_bash(command) {
        if let Some(commands) = try_parse_word_only_commands_sequence(&tree, command) {
...
fn is_command_safe(command: &[String]) -> bool {
    if command.is_empty() {
        return false;
    }
...
/// Extract commands from a bash script
pub fn extract_commands(script: &str) -> Vec<Vec<String>> {
    if let Some(tree) = try_parse_bash(script) {
        if let Some(commands) = try_parse_word_only_commands_sequence(&tree, script) {
            return commands;
        }
    }
    Vec::new()
...
mod tests {
    ...
    fn accepts_multiple_commands_with_allowed_operators() {
        ...
        assert_eq!(cmds.len(), 4); // ls, pwd, echo, wc
    }
    ...
    fn test_safe_command_detection() {
        ...
        assert!(!is_safe_bash_command("sudo ls"));
        assert!(!is_safe_bash_command("echo $(whoami)"));
    }
}