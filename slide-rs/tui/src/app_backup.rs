...
impl App {
        ...
        B: ratatui::backend::Backend,
    {
        ...
        match key {
            ...
            KeyEvent { code: KeyCode::Char('t'), modifiers: KeyModifiers::CONTROL, .. } => {
                ...
                for cell in self.chat_widget.get_history() {
                    for l in cell.lines() {
                        lines.push(l);
                    }
                }
    ...
    fn exec_command_palette(&mut self, idx: usize) {
        ...
        match cmd.as_str() {
            ...
            "Save Chat to slides/draft.md" => match save_chat_as_draft(self.chat_widget.get_history()) {
                Ok(path) => {
    ...
    B: ratatui::backend::Backend,
{
    match ev {
        ...
        CoreEvent::AgentMessageDelta { delta } => {
            // Delegate all delta processing to ChatWidget
        ...
        CoreEvent::ToolBegin { id: _id, kind, summary, cwd: _ } => {
            let label = match kind { 
                CoreToolKind::Exec => SystemLabel::Exec, 
                CoreToolKind::Mcp => SystemLabel::Mcp, 
                CoreToolKind::Search => SystemLabel::Search, 
                CoreToolKind::Info => SystemLabel::Info 
            };
