...
use crate::bottom_pane::selection_popup_common::{GenericDisplayRow, render_rows};
...
impl UserApprovalWidget {
    pub fn new(request: ApprovalRequest, tx: AppEventSender) -> Self {
        ...
        if matches!(request, ApprovalRequest::Patch { .. }) { scroll.selected_idx = Some(0); }
        Self { request, complete: false, tx, scroll, page_size_hint: 10 }
    }
    pub fn handle_key_event(&mut self, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::PageUp => { let step = self.page_size_hint.max(1); self.move_selection_up(step); }
            KeyCode::PageDown => { let step = self.page_size_hint.max(1); self.move_selection_down(step); }
            KeyCode::Home | KeyCode::Char('g') => { self.select_home(); }
            KeyCode::End | KeyCode::Char('G') => { self.select_end(); }
            _ => {}
        }
    }
    pub fn on_ctrl_c(&mut self) { self.complete = true; }
    pub fn is_complete(&self) -> bool { self.complete }
    pub fn desired_height(&self, _width: u16) -> u16 { 10 }
}
    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let header_line: Line = match &self.request {
            ApprovalRequest::Exec { command, reason, .. } => {
                let mut spans = vec![Span::styled("$ ", Style::default().add_modifier(Modifier::BOLD)), Span::raw(command.join(" "))];
                if let Some(r) = reason { spans.push(Span::raw("  — ")); spans.push(Span::styled(r.clone(), Style::default().add_modifier(Modifier::DIM))); }
                Line::from(spans)
            }
            ApprovalRequest::Patch { reason, .. } => {
                let mut spans = vec![Span::styled("apply_patch changes", Style::default().add_modifier(Modifier::BOLD))];
                if let Some(r) = reason { spans.push(Span::raw("  — ")); spans.push(Span::styled(r.clone(), Style::default().add_modifier(Modifier::DIM))); }
                Line::from(spans)
            }
        };
        ...
        if rows_area.height > 0 {
            let rows_all: Vec<GenericDisplayRow> = match &self.request {
                ApprovalRequest::Exec { command, .. } => {
                    vec![GenericDisplayRow { name: format!("$ {}", command.join(" ")), match_indices: None, is_current: false, description: None }]
                }
                ApprovalRequest::Patch { changes, .. } => {
                    changes.iter().map(|ch| GenericDisplayRow { name: ch.clone(), match_indices: None, is_current: false, description: None }).collect()
                }
            };
        ...
        // Footer
        let footer = Rect { x: inner_area.x, y: inner_area.y + inner_area.height.saturating_sub(1), width: inner_area.width, height: 1 };
        ...
        if matches!(self.request, ApprovalRequest::Patch { .. }) {
            ...
            footer_spans.push(Span::styled("↑/↓ PgUp/PgDn Home/End j/k g/G", Style::default().add_modifier(Modifier::DIM)));
        }
...
impl UserApprovalWidget {
    ...
    fn move_selection_up(&mut self, step: usize) {
        if !matches!(self.request, ApprovalRequest::Patch { .. }) { return; }
        let len = self.total_lines();
        if len == 0 { return; }
    ...
    fn select_end(&mut self) {
        if !matches!(self.request, ApprovalRequest::Patch { .. }) { return; }
        let len = self.total_lines();
        if len == 0 { self.scroll.selected_idx = None; self.scroll.scroll_top = 0; return; }
        self.scroll.selected_idx = Some(len - 1);
        self.scroll.scroll_top = self.scroll.scroll_top.max(len.saturating_sub(self.page_size_hint.max(1)));
    }
}
