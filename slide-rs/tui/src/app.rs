...
pub async fn run_app(init_recent_files: Vec<String>) -> Result<RunResult> {
    ...
    // Use TUI event stream for frame synchronization
    ...
    loop {
        ...
        // Handle TUI events with proper ordering
        if let Some(tui_event) = tui_events.next().await {
            match tui_event {
                TuiEvent::Key(key) => {
                ...
                TuiEvent::Draw => {
                    ...
                    let desired_height = app.chat_widget.desired_height(tui.size()?.width);
