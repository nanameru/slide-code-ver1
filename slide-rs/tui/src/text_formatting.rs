...
mod tests {
    ...
    fn test_format_json_compact_nested_object() {
        ...
        assert_eq!(
            result,
            r#"{"user": {"details": {"age": 30, "city": "NYC"}, "name": "John"}}"#
        );
    }
    ...
    fn test_format_json_compact_with_whitespace() {
        ...
        assert_eq!(
            result,
            r#"{"hobbies": ["reading", "coding"], "name": "John"}"#
        );
    }
    ...
    #[test]
    fn test_format_and_truncate_tool_result_json() {
        let json_text = r#"{"name": "John", "age": 30, "city": "New York"}"#;
        let result = format_and_truncate_tool_result(json_text, 2, 30);
        
        // Should format as compact JSON and truncate
        assert!(result.contains("John"));
        assert!(result.len() <= 60); // 2 lines * 30 chars
    }

    #[test]
    fn test_format_and_truncate_tool_result_plain_text() {
        let text = "This is a very long plain text that should be truncated appropriately";
        let result = format_and_truncate_tool_result(text, 1, 20);
        
        // Should truncate to fit within constraints
        assert!(result.len() <= 20);
        assert!(result.ends_with("...") || result.len() == text.len());
    }

    #[test]
    fn test_format_and_truncate_tool_result_empty() {
        let result = format_and_truncate_tool_result("", 5, 80);
        assert_eq!(result, "");
    }
}