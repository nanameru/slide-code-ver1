...
pub fn explain_safety_concern(command: &[String]) -> Option<String> {
    ...
    match cmd0 {
        ...
        Some("mv" | "move") => Some("Command can move or rename files".to_string()),
        Some("cp" | "copy") => {
        ...
        Some("sudo" | "su") => Some("Command attempts privilege escalation".to_string()),
    ...
    for t in toks {
        match t {
            Tok::OpSeq | Tok::OpAnd | Tok::OpOr => {
                // new sequence
                if seqs.last().map(|v| v.is_empty()).unwrap_or(true) {
                    return None;
                } // trailing op
                seqs.push(Vec::new());
            }
