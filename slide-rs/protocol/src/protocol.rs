...
impl WritableRoot {
    pub fn is_path_writable(&self, path: &Path) -> bool {
        path.starts_with(&self.root) && !self.read_only_subpaths.iter().any(|p| path.starts_with(p))
    }
}
